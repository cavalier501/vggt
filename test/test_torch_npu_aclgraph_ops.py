...
def _run_isolated_python(code: str) -> None:
    env = os.environ.copy()
    ...
    if result.returncode != 0:
        pytest.fail(
            "Isolated ACLGraph probe failed.\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )
...
def test_attention_fused_torch_npu_ops_can_capture_raw_aclgraph():
    ...
    _run_isolated_python(
        ...
        def build_positions(batch_size: int, seq_len: int, device: torch.device) -> torch.Tensor:
        ...
        pos = build_positions(batch_size=2, seq_len=17, device=device)
